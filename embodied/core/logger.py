...
class MlflowOutput:
    ...
    def _start_or_resume(self, run_name, resume_id=None):
        ...
        if resume_run_id:
            ...
            print(
                f"Resumed mlflow run {run.info.run_id} ({resume_id}) in experiment {run.info.experiment_id}"
            )
        else:
            run = mlflow.start_run(
                run_name=run_name, tags={"resume_id": resume_id or ""}
            )
            print(
                f"Started mlflow run {run.info.run_id} ({resume_id}) in experiment {run.info.experiment_id}"
            )
        return run
